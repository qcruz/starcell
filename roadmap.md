...
- [ ] Luck system — daily luck value (hidden or hinted via NPC dialogue); affects drop rates, rare encounter chance, crafting success, fishing catch quality
...
- [ ] Favorite system — mark items as favorites to pin them in inventory and flag them as quest tracking targets
...
- [ ] Seasonal system — four seasons: Spring, Summer, Autumn, Winter (~7 in-game days each); season affects crops, wild plants, and NPC behavior
...
- [ ] Village biome — clustered settlement zone with higher NPC density, market stalls, and communal structures; starting point for social and quest arcs

### Zones
- [ ] Zone priority scoring — each instantiated zone has a priority score; connected structures and player-adjacent zones score higher; higher-priority zones update more frequently; zones gain priority with each cycle they go without an update; on update, runs catch-up cycles then regular updates — solves both simulation accuracy and performance
- [ ] Zone and NPC name generation — unique fantasy-language names drawn from generated tables; zone generation influenced by its assigned name

### Travel & Navigation
- [ ] Horse / Mount — rideable; 2× movement speed; requires taming via favor system; can be stabled at a barn/pen structure
- [ ] Waypoint stone — placeable structure; player can teleport between owned waypoints
- [ ] World map view — zoomed-out view showing explored zones and zone names; zone names persist
