...
|---|---|---|---|
| 1 | **Audio System** | 15–20% | Background music (biome-specific, intensity-scaled to nearby combat), ambient sfx (rain, cave drips, fire, wind), action sfx (combat hits, crafting, spells), short NPC vocal cues. **Partially wired:** spatial NPC footstep sounds and creature combat sounds (wolf, goblin, bat, skeleton) are live; the above categories remain open. Largest scope on the list — full system earns top allocation. |
...
| 6 | **Keeper System — Evergael Variant** | 3–5% | Core keeper system is fully live (guard/patrol/zone keeper types, auto-assignment, quest integration, cross-zone pursuit). Remaining open work: the Evergael variant — an ancient or cursed entity trapped in a zone that blocks the exit until defeated, then drops a rare key or artifact. Sprite, AI behavior (ignore player until aggro'd, then zone-to-zone pursuit), and key/artifact drop required. |
| 7 | **Domain Bonuses** | 3–5% | Biome and faction domain systems are fully implemented (BFS contiguity, named domains, save/load, visible in overlay). Remaining open work: gameplay bonuses for being inside a domain — faction domains: increased NPC healing rate, bonus player XP, reduced hostile spawn rate; domain breaks when a zone is captured. Visual domain markers on the overworld also needed. |
| 8 | **Reputation Ripple Effects** | 3–5% | Per-NPC favor score (−100 to 100, updated on attack/gift/trade) is fully live. Remaining open work: global reputation score (player-carried, −100 to 100) — hostile NPCs in a zone flee when player reputation exceeds threshold; traders offer discount at high rep; zone Keeper greets player by name once threshold crossed. No new UI needed — condition checks on existing paths only. |
...
| # | Feature | Est. Pool % | Notes |
|---|---|---|---|
| 16 | **Perk System** | 2–3% | Perks earned through natural gameplay — repeatedly killing goblins builds a goblin-damage bonus, etc. Perks decay slowly over time if not maintained. No trees; perks are flat bonuses earned organically per activity. |
| 17 | **RainbowMaker** | 2–3% | Default sprite colors randomizable via a built-in variant system (e.g. white cow with black spots → red cow with gold spots). Rare color variants create hunting and collecting gameplay. |
| 18 | **Entity Genetics** | 2–3% | Core NPC behaviors are numerically determined and mutate slowly over time (via Contagion System). Genetic variation reflected in sprite color variant chances. |
| 19 | **BugCatcher System Expansion** | 1–2% | Tiered auto-correction responses: zone too dense → re-roll or spawn termite invasion; NPC stuck/teleporting/too high level → force full update + trigger invasion; zone population too far out of balance → LoreEngine procs natural disaster or migration. |
| 20 | **Random Pet / Familiar** | 1–2% | Player spawns each life with a random small pet or familiar follower. Needs a sprite, basic follow behavior, and minor passive effect (e.g. cat reduces nearby pest spawns, bird alerts on approaching hostiles). |
