...
#
# Root cause:  npc_enter_structure() (ai/movement.py) sets entity.in_structure=True
...
#              (ui/hud.py) then draws it with in_structure=True, which causes
#              is_flying_idle to evaluate False → animation freezes at 'still'.
#
# Detection:   entity.in_structure=True AND entity_id in screen_entities[zone_key]
#
# Root-cause patch (applied separately):
#   ui/hud.py render loop — skip drawing entities where player is in overworld
#   but entity.in_structure=True (still run smooth-movement + animation).
#
# Safety-net (apply=True):
#   Clear entity.in_structure=False so animation resumes immediately.
...
def fix_entity_subscreen_flag(entity_id, entity, game, bug_catcher=None,
    ...
    """Detect (and optionally correct) entity with in_structure=True stuck in
    screen_entities.
    ...
    apply     : if True, clear in_structure=False as a safety net
    ...
    desc = (
        f"entity {entity_id} ({entity.type}) has in_structure=True "
        f"but is still in screen_entities[{zone_key}]"
        + (f" — in_structure cleared" if apply else " — detected only")
    )
    ...
    if apply:
        # Remove from screen_entities before clearing the key
        old_sub_key = getattr(entity, 'structure_key', None)
        if old_sub_key is not None:
            sub_list = getattr(game, 'screen_entities', {}).get(old_sub_key, [])
