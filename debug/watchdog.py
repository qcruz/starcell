...
The Watchdog runs every SAMPLE_INTERVAL ticks (~5 s at 60 fps).  Each cycle it:
  1. Samples ONE category comprehensively — logging ALL available items in that
     category (all entities, all zones, all subscreens, etc.).
...
Categories (rotating):
  entities   — ALL entities: full health/position/AI/subscreen/combat state
  ...
  subscreens — ALL subscreens: type, depth, entity list, cell histogram
...
class Watchdog:
    ...
    def _sample_player(self, tick: int, game) -> None:
        ...
        if proxy_id and proxy_id in game.entities:
            ...
            proxy_pos = {
                ...
                'in_subscreen': getattr(px, 'in_subscreen', False),
            }
        self.bug_catcher.log({
            'tick': tick,
    ...
    def _sample_subscreens(self, tick: int, game) -> None:
        """Log full state for ALL subscreens (type, cells, entities)."""
        if not game.subscreens:
            ...
            return
        all_items = list(game.subscreens.items())
        selected = self._trim(tick, 'subscreens', all_items)
        subscreen_entities = getattr(game, 'subscreen_entities', {})
        for sub_key, sub_data in selected:
            entity_ids = subscreen_entities.get(sub_key, [])
            entity_details = []
            for eid in entity_ids:
                if eid in game.entities:
                    e = game.entities[eid]
    ...
    def _check_integrity(self, tick: int, game) -> None:
        ...
        # Build reverse map: entity_id -> [subscreen_keys it appears in]
        entity_in_subs: dict = {}
